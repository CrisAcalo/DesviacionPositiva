        ...
        <Card className="border-blue-200 bg-blue-50/50 dark:border-blue-900 dark:bg-blue-950/20">
            ...
            <CardContent className="space-y-3 text-sm">
                <p className="text-muted-foreground">
                    Se aceptan archivos <strong>CSV</strong> y <strong>Excel (.xlsx, .xls)</strong>. La primera fila debe ser el encabezado.
                </p>
                ...
                <ul className="space-y-1 text-muted-foreground">
                    ...
                    <li>• En CSV se acepta coma (<code>,</code>) o punto y coma (<code>;</code>) como delimitador.</li>
                </ul>
            </CardContent>
        </Card>
    );
...
export default function NrcCreate({ departments, careers, subjects, academicPeriods }: PageProps) {
        ...
        return new Promise((resolve) => {
            Papa.parse<Record<string, string>>(file, {
                ...
                complete: (result) => {
                    ...
                    resolve({
                        file,
                        preview: result.data.slice(0, 8) as Record<string, string>[],
                        headers,
    ...
    const updateMeta = (index: number, field: keyof FileEntry['meta'], value: string) => {
        setEntries((prev) => {
            const next = [...prev];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!academicPeriod) errors.academic_period = 'El período académico es obligatorio.';
        if (entries.length === 0) errors.files = 'Agrega al menos un archivo.';

        entries.forEach((entry, i) => {
            if (!entry.meta.code) errors[`uploads.${i}.code`] = 'Requerido.';
            if (!entry.meta.subject_id) errors[`uploads.${i}.subject_id`] = 'Requerido.';
            if (!entry.meta.career_id) errors[`uploads.${i}.career_id`] = 'Requerido.';
            if (entry.parseErrors.length > 0) errors[`uploads.${i}.file`] = 'El archivo tiene errores de formato.';
        });
    ...
    const allValid = entries.length > 0 && entries.every((e) => e.parseErrors.length === 0);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Card>
                        <CardHeader className="pb-3">
                            <CardTitle className="text-sm font-medium">Período académico</CardTitle>
                        ...
                        <CardContent>
                            <div className="max-w-xs grid gap-1.5">
                                <Input
                                    ...
                                    className={formErrors.academic_period ? 'border-destructive' : ''}
                                />
                                <p className="text-xs text-muted-foreground">
                                    Puedes usar cualquier formato. Ejemplos: "2024-2025", "2024 P1", "Semestre 2024-2"
                                </p>
                                {formErrors.academic_period && (
                                    <p className="text-xs text-destructive">{formErrors.academic_period}</p>
                                )}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    >
                        ...
                        <div className="text-center">
                            <p className="font-medium">Arrastra archivos CSV o Excel aquí</p>
                            <p className="text-sm text-muted-foreground">o haz clic para seleccionar — puedes subir varios a la vez</p>
                        </div>
                        <input
                            ref={fileInputRef}
                            type="file"
                    ...
                    {entries.map((entry, index) => (
                        <Card key={`${entry.file.name}-${index}`} className={entry.parseErrors.length > 0 ? 'border-destructive' : ''}>
                            <CardHeader className="pb-3">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <span className="truncate font-medium text-sm">{entry.file.name}</span>
                                    ...
                                    <Alert variant="destructive">
                                        ...
                                        <AlertDescription>
                                            <ul className="list-disc pl-4 text-sm">
                                                {entry.parseErrors.map((err, i) => <li key={i}>{err}</li>)}
                                            </ul>
                                        </AlertDescription>
                                    </Alert>
                                ) : (
                                    <>
                                        ...
                                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                            <div className="grid gap-1.5">
                                                <Label className="text-xs">Código NRC *</Label>
                                                <Input
                                                    placeholder="Ej: 12345"
                                                    value={entry.meta.code}
                                                    onChange={(e) => updateMeta(index, 'code', e.target.value)}
                                                    className="h-8 text-sm"
                                                />
                                                {formErrors[`uploads.${index}.code`] && (
                                                    <p className="text-xs text-destructive">{formErrors[`uploads.${index}.code`]}</p>
                                                )}
                                            </div>
                                            ...
                                            <div className="grid gap-1.5">
                                                ...
                                                <Select
                                                    value={entry.meta.subject_id}
                                                    onValueChange={(v) => updateMeta(index, 'subject_id', v)}
                                                >
                                                ...
                                                {formErrors[`uploads.${index}.subject_id`] && (
                                                    <p className="text-xs text-destructive">{formErrors[`uploads.${index}.subject_id`]}</p>
                                                )}
                                            </div>
                                            ...
                                            <div className="grid gap-1.5 lg:col-span-2">
                                                ...
                                                <Select
                                                    ...
                                                    onValueChange={(v) => updateMeta(index, 'career_id', v)}
                                                >
                                                    <SelectTrigger className="h-8 text-sm">
                                                        <SelectValue placeholder="Seleccionar…" />
                                                    </SelectTrigger>
                                                ...
                                                {formErrors[`uploads.${index}.career_id`] && (
                                                    <p className="text-xs text-destructive">{formErrors[`uploads.${index}.career_id`]}</p>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        {entry.preview.length > 0 && (
                                            <div>
                                                <p className="mb-2 flex items-center gap-1.5 text-xs text-muted-foreground">
                                                    ...
                                                    Vista previa — primeras {entry.preview.length} filas
                                                </p>
                                                <div className="overflow-x-auto rounded-lg border">
                                                    <Table>
                                                        <TableHeader>
                                                            <TableRow>
                                                                {entry.headers.map((h) => (
                                                                    <TableHead key={h} className="whitespace-nowrap py-2 text-xs">
                                                                        {h}
                                                                    </TableHead>
                                                                ))}
                                                            </TableRow>
                                                        ...
                                                        <TableBody>
                                                            {entry.preview.map((row, i) => (
                                                                <TableRow key={i}>
                                                                    {entry.headers.map((h) => (
                                                                        <TableCell key={h} className="py-1.5 font-mono text-xs">
                                                                            {String(row[h] ?? '')}
                                                                        </TableCell>
                                                                    ))}
                                                                </TableRow>
                    ...
                    {entries.length > 0 && (
                        <div className="flex items-center justify-between rounded-xl border bg-muted/30 px-4 py-3">
                            <span className="text-sm text-muted-foreground">
                                {entries.length} archivo{entries.length !== 1 ? 's' : ''} listo{entries.length !== 1 ? 's' : ''} para importar
                            </span>
                            <Button type="submit" disabled={submitting || !allValid || !academicPeriod}>
                                {submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                Importar {entries.length > 1 ? `${entries.length} NRCs` : 'NRC'}
                            </Button>
                        </div>
                    )}
                </form>
